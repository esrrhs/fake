...
void optimizer::optimize_no_use_stack(func_binary & fb)
{
	for (int i = 0; i < fb.m_debug_stack_variant_info_num; i++)
	{
		...
		if (!use)
		{
			stack_variant_info * newlist = (stack_variant_info *)safe_fkmalloc(m_fk, (fb.m_debug_stack_variant_info_num - 1) * sizeof(stack_variant_info), emt_func_binary);
			for (int z = 0; z < i; z++)
			{
...
void optimizer::optimize_no_use_container(func_binary & fb)
{
	for (int i = 0; i < fb.m_container_addr_list_num; i++)
	{
		...
		if (!use)
		{
			...
			container_addr * newlist = (container_addr *)safe_fkmalloc(m_fk, (fb.m_container_addr_list_num - 1) * sizeof(container_addr), emt_func_binary);
			for (int j = 0; j < i; j++)
			{
...
void optimizer::optimize_no_use_const(func_binary & fb)
{
	for (int i = 0; i < fb.m_const_list_num; i++)
	{
		...
		{
			command cmd = fb.m_buff[j];
			if (is_addr_type(cmd, ADDR_CONST, i))
			{
				use = true;
				break;
			}
		...
		{
			container_addr & addr = fb.m_container_addr_list[j];
			if (is_addr_type(addr.con, ADDR_CONST, i))
			{
				use = true;
				break;
			}
			if (is_addr_type(addr.key, ADDR_CONST, i))
			{
				use = true;
				break;
			}
		...
		if (!use)
		{
			for (int j = i; j < fb.m_const_list_num - 1; j++)
			{
				replace_all_addr(fb, ADDR_CONST, j + 1, j);
			}

			variant * newlist = (variant *)safe_fkmalloc(m_fk, (fb.m_const_list_num - 1) * sizeof(variant), emt_func_binary);
			for (int j = 0; j < i; j++)
			{
...
opt_ins * optimizer::get_assign_dst_ins_from(int pos, command cmd)
{
	for (int i = pos; i < (int)m_inslist.size(); i++)
	{
		opt_ins & ins = m_inslist[i];
...
opt_ins * optimizer::get_write_ins_from(int pos, command cmd)
{
	for (int i = pos; i < (int)m_inslist.size(); i++)
	{
		opt_ins & ins = m_inslist[i];
